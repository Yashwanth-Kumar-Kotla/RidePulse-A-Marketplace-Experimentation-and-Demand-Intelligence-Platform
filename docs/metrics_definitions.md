...
| 11 | Surge proxy | zone-hour | Fare ($/mile) vs. median $/mile for comparable trips (same zone, hour-of-day, day-of-week, across all observed weeks in the warehouse) | Detects de facto price elevation without an explicit surge multiplier field | HVFHS doesn't expose Uber's/Lyft's actual surge multiplier; this is inferred from realized fares, so it conflates surge with genuine trip-mix shifts (longer/shorter trips). Measured on the full 2023-2025 window: median ratio is exactly 1.0 by construction, p10-p90 spread is 0.83-1.24 | Built | `kpi_surge_proxy.sql` |
| 12 | Week-over-week deltas + anomaly flags | zone / hour-of-day / day-of-week | Robust z-score: `(this_week_count - median_count) / (1.4826 * MAD)` across all observed weeks for that (zone, hour, dow) | Cheap anomaly surfacing without a full time-series model | Originally built and measured against the 3-month pilot window (8-14 weeks/group, thin but workable for MAD). Re-verified on the full 2023-2025 window: 1-131 weeks observed per group (avg ~129) -- much more robust. Measured anomaly rate dropped from 3.9% (pilot) to 2.2% (full window), consistent with a more stable MAD estimate from more history reducing small-sample false flags | Built | `kpi_wow_anomaly.sql` |
